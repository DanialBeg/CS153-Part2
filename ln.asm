...
main(int argc, char *argv[])
{
  ...
  if(argc != 3){
    ...
    101a:	68 54 17 00 00       	push   $0x1754
    101f:	6a 02                	push   $0x2
  ...
  if(link(argv[1], argv[2]) < 0)
    ...
    104a:	68 67 17 00 00       	push   $0x1767
    104f:	6a 02                	push   $0x2
...
printint(int fd, int xx, int base, int sgn)
{
  ...
  do{
    ...
    139f:	f7 f6                	div    %esi
    13a1:	0f b6 92 84 17 00 00 	movzbl 0x1784(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    1518:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
    151b:	b8 7b 17 00 00       	mov    $0x177b,%eax
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15a1:	a1 60 1a 00 00       	mov    0x1a60,%eax
static Header base;
static Header *freep;
...
free(void *ap)
{
  ...
  freep = p;
    15f9:	a3 60 1a 00 00       	mov    %eax,0x1a60
}
    15fe:	5b                   	pop    %ebx
    15ff:	5e                   	pop    %esi
    1600:	5f                   	pop    %edi
  ...
  if(p + p->s.size == bp){
    ...
    p->s.ptr = bp;
  freep = p;
    161d:	a3 60 1a 00 00       	mov    %eax,0x1a60
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    163c:	8b 15 60 1a 00 00    	mov    0x1a60,%edx
malloc(uint nbytes)
{
  Header *p, *prevp;
  uint nunits;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    16dc:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
    16df:	89 15 60 1a 00 00    	mov    %edx,0x1a60
      return (void*)(p + 1);
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
    16f6:	c7 05 60 1a 00 00 64 	movl   $0x1a64,0x1a60
    16fd:	1a 00 00 
    1700:	c7 05 64 1a 00 00 64 	movl   $0x1a64,0x1a64
    1707:	1a 00 00 
    base.s.size = 0;
    170a:	b8 64 1a 00 00       	mov    $0x1a64,%eax
    170f:	c7 05 68 1a 00 00 00 	movl   $0x0,0x1a68
    1716:	00 00 00 
